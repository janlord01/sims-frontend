<template>
  <q-page style="margin-right: 10px">
    <table style="font-size: 8px; width: 40%; margin-left: 1%">
      <tbody>
        <tr class="">
          <td class="text-align:left;width: 100px;">
            ...
            <!-- <q-icon v-else left name="account_circle" size="100px" /> -->
          </td>
    ...
    >
      <thead>
        <tr>
          <th style="border: 1px solid #000; width: 80px">Date</th>
          <th style="border: 1px solid #000; width: 50px">Ref #</th>
      ...
      <tbody>
        <tr v-for="item in payables">
          <td
            ...
            v-if="item.subject_id && item.level_id > 14"
          >
        ...
        <!-- Billings -->
        <tr v-for="item in payables">
          <td style="text-align: center" v-if="item.billing_id">
            <span style="display: block">
              {{
                item.date != null
                  ? moment(item.date).format("MMMM D, YYYY")
          ...
          <td style="text-align: left" v-if="item.billing_id">
            <span style="display: block; text-align: center">
              {{ item.ref }}
            </span>
          </td>
          <td style="text-align: right" v-if="item.billing_id">
            <span style="display: block; text-align: left">
              {{ item.description }}
            </span>
          </td>
        ...
        <tr>
          ...
          <td style="text-align: right">
            <span style="display: block; text-align: left"> Total </span>
          </td>
          <td style="text-align: right">
            <span style="display: block; text-align: left"> ---------- </span>
          </td>
        ...
        <tr v-for="item in paymentTransactions">
          <td style="text-align: center">
            <span style="display: block">
              {{
                item.date != null
        ...
        <tr v-for="item in due" style="">
          ...
          <td style="text-align: right">
            <span style="display: block; font-weight: bolder; text-align: left">
              {{ item.description }}
            </span>
          </td>
          <td style="text-align: right">
        ...
        <div style="margin-top: 20px"></div>
        <tr v-for="item in discounts" v-if="discounts.length > 0">
          <td style="text-align: center">
            <span style="display: block">
              {{
                item.date != null
        ...
        <tr v-if="discounts.length > 0">
          ...
          <td style="text-align: right">
            <span style="display: block; color: red; text-align: left">
              <!-- {{ item.description }} -->
              Please Note: Schoolarship/Subsidy will be added to student
              credits/wallet, he/she can use it for payment transactions
            </span>
          </td>
          <td style="text-align: right">
            <span style="display: block; text-align: right">
    ...
    <table
      style="width: 40%; margin-top: 20px; font-size: 8px; margin-left: 1%"
    >
      <tr>
        <th></th>
        <th>
...
export default defineComponent({
  ...
  methods: {
    ...
    async printSoa() {
      ...
      await api
        .get("/api/admission/print-soa/" + this.$route.params.admission_id, {
          headers: {
        ...
        .then((response) => {
          ...
          Object.entries(response.data.paymentTransactions).map(
            ([key, val]) => {
          ...
          Object.entries(response.data.balance).map(([key, val]) => {
            this.balance.push({
              date: val.created_at,
              description: "Balance",
              charges: val.pre_balance,
            });
          });
          Object.entries(response.data.admission).map(([key, val]) => {
            this.payables.push({
              date: val.created_at,
              description: val.descr != null ? val.code : val.name,
              charges: val.total,
              subject_id: val.subject_id,
              billing_id: val.billing_id,
              level_id: val.level_id,
            });
            this.totalPay = parseFloat(this.totalPay) + parseFloat(val.total);
          });
          // console.log(response.data.admission);
    ...
    print() {
      ...
      var doc = new jsPDF("portrait", "pt", "legal");
