<template>
  <q-card style="max-width: 500px; width: 500px">
    ...
    <q-toolbar class="bg-secondary text-white">
      <q-toolbar-title> Edit Requirement </q-toolbar-title>
...
export default {
  ...
  methods: {
    async editReq() {
      await api
        .get("/api/settings/requirement/" + this.reqId + "/edit", {
          headers: {
        ...
        .then((response) => {
          ...
          this.formData.title = response.data.requirement.title
            ? response.data.requirement.title
            : "";
    ...
    async onSubmit() {
      await api
        .patch(
          "/api/settings/requirement/" + this.reqId,
          {
