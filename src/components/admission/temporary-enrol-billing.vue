<template>
  ...
  <q-table
    ...
    row-key="id"
    separator="cell"
    :visible-columns="$q.screen.gt.xs ? ['name', 'price', 'action'] : ['name', 'price']"
  >
    <template #body="props">
      <!-- <p v-for="item in studentSubjects"> -->
      <q-tr :props="props" :class="props.row.id === enrollment_fee_id ? 'bg-red-5' : ''">
        <!-- {{ item.code }}
        {{ props.row.name }} -->
        ...
        <q-td key="action" :props="props">
          <q-btn
            ...
            icon="delete"
            size="sm"
            v-if="props.row.id === enrollment_fee_id"
          />
          <q-btn
            color="red"
            icon="delete"
...
export default {
    ...
    async getStudentData() {
      await api
        ...
        .then((response) => {
          ...
          this.enrollment_fee_id = response.data.billings[0].id;
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  mounted() {
    ...
    this.getBalance([this.$route.params.student_id, "save"]);
    this.getStudentData();
